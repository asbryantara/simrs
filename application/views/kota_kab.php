...
?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        <i class="fa fa-circle-o"></i> &nbsp; Data Kota / Kabupaten
        <small></small>
      </h1>
      <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title" id="judul-modal">Tambah Data kota_kab</h4>
            </div>
            <div class="modal-body">
              ...
              <div class="form-group">
                ...
                <div class="col-md-8">
                  <input type="text" name="nama_kota_kab" id="nama_kota_kab" class="form-control" autocomplete="off" placeholder="ex: KABUPATEN PASURUAN">
                </div>
              </div>
              <br>
              <br>
      ...
      <div class="modal fade bd-example-modal-lg modal-edit" tabindex="false" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title">Edit Data kota_kab</h4>
            </div>
            <div class="modal-body">
              <div class="form-group">
                <label class="control-label col-md-3">Nama Provinsi</label>
              ...
              <br>
              <div class="form-group">
                <label class="control-label col-md-3">Nama kota_kab</label>
                <div class="col-md-8">
                  <input type="text" name="no_rm" id="u_nama_kota_kab" class="form-control" autocomplete="off">
                  <input type="hidden" name="u_id_kota_kab" id="u_id_kota_kab">
                </div>
              </div>
      ...
      <div class="modal fade" id="modal-hapus" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <input type="hidden" name="d_id_kota_kab" id="d_id_kota_kab">
              <center>
...
<script>
  $(document).ready(function(){
    ...
    function tampil(){
      $.ajax({
        type: 'GET',
        url: '<?= base_url('master/show_kota_kab') ?>',
        dataType: 'JSON',
    ...
    $('#nama_kota_kab').on('keyup', function(e){
        if(e.keyCode == 13){
          simpan();
        }
    });
    ...
    function simpan(){
      ...
      $.ajax({
        type: 'POST',
        url: '<?= base_url('master/save_kota_kab') ?>',
        dataType: 'JSON',
        data:{
        ...
        success: function(data){
          if(data == 'duplikasi'){
            alert(nama_kota_kab+' sudah ada di dalam database !');
          }else{
            $('#nama_kota_kab').val('');
    ...
    $(document).on('click', '.btn-edit', function(){
      var id_kota_kab = $(this).attr('data');
      $.ajax({
        type: 'POST',
        url: '<?= base_url('master/detail_kota_kab') ?>',
        dataType: 'JSON',
        data: {id_kota_kab:id_kota_kab},
        success: function(data){
          $('#u_id_kota_kab').val(data.id_kota_kab);
          $('#u_id_provinsi').val(data.id_provinsi).trigger('change');
          $('#u_nama_kota_kab').val(data.nama_kota_kab);
    ...
    $('#u_nama_kota_kab').on('keyup', function(e){
      if(e.keyCode == 13){
        update();
      }
    });
    ...
    function update(){
      ...
      var nama_kota_kab = $('#u_nama_kota_kab').val();
      $.ajax({
        type: 'POST',
        url: '<?= base_url('master/update_kota_kab') ?>',
        dataType: 'JSON',
        data:{
    ...
    $(document).on('click', '#hapus', function(){
      var id_kota_kab = $('#d_id_kota_kab').val()
      $.ajax({
        type: 'POST',
        url: '<?= base_url('master/delete_kota_kab') ?>',
        dataType: 'JSON',
        data:{id_kota_kab:id_kota_kab},
        success: function(data){
    ...
    $('#cari').on('keyup', function(e){
      ...
      $.ajax({
        type: 'POST',
        url: '<?= base_url('master/search_kota_kab') ?>',
        dataType: 'JSON',
        data:{search:search},
        success: function(data){
          sukses(data);
        ...
        for(i=0; i<data.length; i++){
          html += `
            <tr>
              ...
              <td>
                <button type="button" class="btn btn-warning btn-xs btn-edit" data="`+data[i].id_kota_kab+`">Edit</button>
                <button type="button" class="btn btn-danger btn-xs btn-hapus" data="`+data[i].id_kota_kab+`">Hapus</button>
